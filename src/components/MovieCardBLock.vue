<template>
	<div class="relative">
		...
		<div class="py-20">
			<div
				class="bg-gray-950/95 text-white relative w-3/4 m-auto z-1 flex flex-row rounded-lg"
			>
				<img
					class="w-1/3"
				...
				<div class="p-10">
					...
					<div class="grid grid-cols-2 grid-rows-2 gap-3 pt-2">
						<p>
							Film duration:
							<strong class="text-xl text-emerald-500"
						...
						<p>
							Vote average:
							<strong class="text-xl text-emerald-500"
								>{{ moviesStore.movieInfo.vote_average }}/10</strong
							>
						</p>
					</div>
				</div>
...
<script setup>
	...
	function convertRevenue(sum) {
		let result = `${(sum / 1000000).toFixed(2)} mln$`
		return result
	}
